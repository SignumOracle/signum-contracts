...
contract SignumTributes is TellorStorage, TellorVars {
    ...
    /**
     * @notice Allows signum team to transfer stake of disputed signum reporter
     * NOTE: this does not affect SignumFlex stakes, only disputes during 360 transition period
     ...
     * and removes the staked amount if they are staked in Signum and disputed
        ...
        returns (bool)
    {
        if (stakerDetails[_user].currentStatus == 3) {
            // Subtracts the stakeAmount from balance if the _user is staked and disputed in Signum
     ...
     * and ensuring the amount does not contain tokens locked for signum disputes
